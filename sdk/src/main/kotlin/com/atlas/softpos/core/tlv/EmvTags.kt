...
object EmvTags {
    ...
    // These aliases provide backward compatibility with code using full tag names

    /** Alias: TERMINAL_VERIFICATION_RESULTS -> TVR */
    val TERMINAL_VERIFICATION_RESULTS = TVR

    /** Alias: APPLICATION_TRANSACTION_COUNTER -> ATC */
    val APPLICATION_TRANSACTION_COUNTER = ATC

    /** Alias: CRYPTOGRAM_INFO_DATA -> CID */
    val CRYPTOGRAM_INFO_DATA = CID

    /** Alias: ISSUER_APPLICATION_DATA -> IAD */
    val ISSUER_APPLICATION_DATA = IAD

    /** Alias: EXPIRY_DATE -> APP_EXPIRY_DATE */
    val EXPIRY_DATE = APP_EXPIRY_DATE

    /** Alias: EFFECTIVE_DATE -> APP_EFFECTIVE_DATE */
    val EFFECTIVE_DATE = APP_EFFECTIVE_DATE

    /** Alias: ADDITIONAL_TERMINAL_CAPABILITIES -> ADDITIONAL_TERMINAL_CAPS */
    val ADDITIONAL_TERMINAL_CAPABILITIES = ADDITIONAL_TERMINAL_CAPS

    /** Alias: MERCHANT_CATEGORY_CODE -> MCC */
    val MERCHANT_CATEGORY_CODE = MCC

    /** Alias: TERMINAL_IDENTIFICATION -> TERMINAL_ID */
    val TERMINAL_IDENTIFICATION = TERMINAL_ID

    /** Alias: ACQUIRER_IDENTIFIER -> ACQUIRER_ID */
    val ACQUIRER_IDENTIFIER = ACQUIRER_ID

    /** Alias: APPLICATION_VERSION_NUMBER -> APP_VERSION_TERMINAL */
    val APPLICATION_VERSION_NUMBER = APP_VERSION_TERMINAL

    /** Alias: SIGNED_STATIC_APPLICATION_DATA -> SSAD */
    val SIGNED_STATIC_APPLICATION_DATA = SSAD

    /** Alias: ISSUER_PUBLIC_KEY_CERTIFICATE -> ISSUER_PK_CERTIFICATE */
    val ISSUER_PUBLIC_KEY_CERTIFICATE = ISSUER_PK_CERTIFICATE

    /** Alias: ISSUER_PUBLIC_KEY_EXPONENT -> ISSUER_PK_EXPONENT */
    val ISSUER_PUBLIC_KEY_EXPONENT = ISSUER_PK_EXPONENT

    /** Alias: ISSUER_PUBLIC_KEY_REMAINDER -> ISSUER_PK_REMAINDER */
    val ISSUER_PUBLIC_KEY_REMAINDER = ISSUER_PK_REMAINDER

    /** Alias: ICC_PUBLIC_KEY_CERTIFICATE -> ICC_PK_CERTIFICATE */
    val ICC_PUBLIC_KEY_CERTIFICATE = ICC_PK_CERTIFICATE

    /** Alias: ICC_PUBLIC_KEY_EXPONENT -> ICC_PK_EXPONENT */
    val ICC_PUBLIC_KEY_EXPONENT = ICC_PK_EXPONENT

    /** Alias: ICC_PUBLIC_KEY_REMAINDER -> ICC_PK_REMAINDER */
    val ICC_PUBLIC_KEY_REMAINDER = ICC_PK_REMAINDER

    /** Alias: STATIC_DATA_AUTHENTICATION_TAG_LIST -> SDA_TAG_LIST */
    val STATIC_DATA_AUTHENTICATION_TAG_LIST = SDA_TAG_LIST
}
